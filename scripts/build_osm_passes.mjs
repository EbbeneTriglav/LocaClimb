...
async function main() {
  ...
  for (let ti = 0; ti < cells.length; ti++) {
    ...
    const HWRE = "motorway|trunk|primary|secondary|tertiary|unclassified|residential|living_street|service|track|cycleway";
    const q = '[out:json][timeout:90];node["mountain_pass"="yes"](' + a + ',' + b + ',' + c + ',' + d + ')->.p;way(around.p:150)["highway"~"^(' + HWRE + ')$"]->.w;.p out body;.w out geom tags;';
    try {
