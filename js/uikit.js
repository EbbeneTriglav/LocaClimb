/* uikit.js v2 — ridisegno interfaccia LocaRide. Modulo AUTONOMO e reversibile
 * (togli la riga <script src="js/uikit.js"> e torna tutto com'era).
 *
 *  1) BARRA SNELLA: in vista Filtri + "Crea giro" (primario) + Altro. Traffico/Editor/Tema/
 *     Lingua/Account/I miei giri dentro "Altro" (menu ATTACCATO AL BODY, mai sotto la mappa).
 *  2) ACQUA + CIBO: due pulsanti FISSI sulla mappa, sempre visibili (scorrono di lato quando
 *     si apre la scheda salita). L'acqua richiama la funzione esistente; il cibo carica i
 *     ristori (bar/panetterie/market) nel riquadro visibile mentre navighi.
 *  3) RICERCA UNICA: i "Luoghi" (Nominatim) sotto i passi nella stessa tendina; scegliendo un
 *     luogo la mappa ci vola e i PASSI RESTANO visibili (niente mappa vuota).
 *  4) NAV MOBILE a schede in basso.  5) LEGENDA difficolta' in basso-centro + hint primo avvio.
 */
(function () {
  "use strict";
  ...
  function injectStyle() {
    ...
    var css =
      ...
      "#uk-mapctl{position:fixed;right:14px;top:calc(var(--hdr) + 90px);z-index:900;display:flex;flex-direction:column;gap:9px;transition:transform .28s ease}" +
      "#uk-mapctl .btn{width:46px;height:46px;border-radius:50%;padding:0;justify-content:center;background:var(--bg2);box-shadow:0 3px 12px rgba(0,0,0,.18)}" +
      "#uk-mapctl .btn .bl{display:none!important}#uk-mapctl .btn .bi{font-size:1.35rem}" +
      "#uk-mapctl .btn.active{background:var(--ac);color:#fff}" +
      "body.dp-open #uk-mapctl{transform:translateX(-532px)}" +
      "@media(max-width:900px){body.dp-open #uk-mapctl{transform:none}}" +
      "#uk-legend{position:fixed;left:50%;transform:translateX(-50%);bottom:14px;z-index:901;background:var(--bg2);border:1px solid var(--bdr);border-radius:10px;padding:6px 11px;font-size:.72rem;color:var(--txt2);box-shadow:0 4px 14px rgba(0,0,0,.14);display:flex;gap:9px;align-items:center;flex-wrap:wrap}" +
      ...
      "#uk-legend{bottom:76px}#uk-mapctl{top:auto;bottom:150px}#be-btn{bottom:74px}}";
    document.head.appendChild(el("style", { id: "uk-style" }, css));
  }

  /* ---- 1) menu Altro (attaccato al body, mai sotto la mappa) ---- */
  function buildMore() {
    ...
    ["tb", "ed", "db", "langBtn", "myrides", "acct"].forEach(function (id) { var b = byId(id); if (b) menu.appendChild(b); });
    ...
    btn.addEventListener("click", function (e) { e.stopPropagation(); var open = menu.classList.toggle("open"); if (open) place(); });
  ...
  /* ---- 2) acqua + cibo fissi sulla mappa ---- */
  ...
  function ovPost(q) {
    return new Promise(function (res, rej) {
      var i = 0;
      (function go() {
        if (i >= OVMIR.length) { rej(); return; }
        var ac = ("AbortController" in window) ? new AbortController() : null, to = ac ? setTimeout(function () { ac.abort(); }, 12000) : null;
        fetch(OVMIR[i++], { method: "POST", body: "data=" + encodeURIComponent(q), signal: ac ? ac.signal : undefined })
          .then(function (r) { if (to) clearTimeout(to); if (!r.ok) throw 0; return r.json(); }).then(res).catch(function () { go(); });
      })();
    });
  }
  ...
  function loadFood() {
    ...
    ovPost(q).then(function (d) {
      (d.elements || []).forEach(function (e2) {
        if (e2.type !== "node" || foodIds[e2.id]) return; foodIds[e2.id] = 1; var t = e2.tags || {};
        var ic = L.divIcon({ className: "uk-food-ic", html: '<div style="font-size:15px;filter:drop-shadow(0 1px 1px rgba(0,0,0,.4))">&#x1F37D;&#xFE0F;</div>', iconSize: [20, 20], iconAnchor: [10, 10] });
        L.marker([e2.lat, e2.lon], { icon: ic }).bindPopup("&#x1F37D;&#xFE0F; <b>" + esc(t.name || "Ristoro") + "</b><br>" + esc(t.amenity || t.shop || "")).addTo(foodLayer);
      });
    }).catch(function () {});
  }
  function toggleFood(btn) {
    var map = getMap(); if (!map) return;
    foodOn = !foodOn; if (btn) btn.classList.toggle("active", foodOn);
    if (foodOn) { map.on("moveend", foodMove); loadFood(); }
    else { map.off("moveend", foodMove); if (foodLayer) foodLayer.clearLayers(); foodIds = {}; }
  }
  function buildMapCtl() {
    if (byId("uk-mapctl")) return;
    var box = el("div", { id: "uk-mapctl" });
    var fount = byId("fountBtn"); if (fount) box.appendChild(fount); // riporta l'acqua sulla mappa, sempre visibile
    ...
    box.appendChild(food); document.body.appendChild(box);
  }

  /* ---- 3) ricerca unica ---- */
  var stimer = null, sctrl = null;
  function restorePasses() {
    var q = byId("search"); if (q) q.value = "";
    var af = gref("applyFilters"); if (typeof af === "function") { try { af(); } catch (e) {} }
  ...
  function goPlace(r) {
    ...
    restorePasses(); // i passi restano tutti visibili
  }
  function placeAppend() {
    ...
    fetch("https://nominatim.openstreetmap.org/search?format=jsonv2&limit=4&accept-language=it&q=" + encodeURIComponent(v), { signal: sctrl ? sctrl.signal : undefined, headers: { "Accept": "application/json" } })
      .then(function (r) { return r.ok ? r.json() : []; })
      .then(function (list) {
        if (!Array.isArray(list) || !list.length || q.value.trim() !== v) return;
        var box = byId("sresults"); if (!box) return;
        var old = box.querySelector(".uk-secwrap"); if (old) old.remove();
        var wrap = el("div", { "class": "uk-secwrap" }); wrap.appendChild(el("div", { "class": "uk-sec" }, "Luoghi"));
        list.forEach(function (r) {
          var it = el("div", { "class": "uk-place" }, "<span>&#x1F4CD; " + esc(r.name || (r.display_name || "").split(",")[0]) + "</span><small>" + esc(r.display_name || "") + "</small>");
  ...
  function buildLegend() {
    ...
    var g = el("div", { id: "uk-legend" }); var html = "<b>Difficolta</b>";
  ...
  function firstHint() {
    ...
    var h = el("div", { id: "uk-hint" }, "&#x1F44B; I pallini colorati sono le salite: il <b>colore</b> e' la difficolta (HC = piu dura). Tocca un pallino per i dettagli, o usa <b>Crea giro</b> per un percorso. In alto trovi acqua e ristori.<br><button id='uk-hint-ok'>Ho capito</button>");
    document.body.appendChild(h);
    var ok = byId("uk-hint-ok"); if (ok) ok.addEventListener("click", function () { try { localStorage.setItem("uk_hint_seen", "1"); } catch (e) {} h.remove(); });
  }
  function relabelRoute() { var rb = byId("rbb"); if (rb) { var bl = rb.querySelector(".bl"); if (bl) bl.textContent = "Crea giro"; } }

  function start() {
    injectStyle();
    [buildMore, buildMapCtl, wireSearch, buildTabs, buildLegend, firstHint, relabelRoute].forEach(function (f) { try { f(); } catch (e) {} });
  }
