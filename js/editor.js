...
function edAddPoint(la,lo){var p={lat:la,lon:lo};ED.track.splice(edBestInsert(p),0,p);ED.markers.push(edMakeMarker(p));edRebuild();edRedraw();edStale();}
...
function edRenderList(){var box=document.getElementById("edList");if(!box)return;box.innerHTML="";ED.versanti.forEach(function(v,i){var r=document.createElement("div");r.style.cssText="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px solid #f3f3f3";r.innerHTML="<span>"+esc(v.side)+" &middot; "+v.distance_km+"km "+v.avgGradient+"%</span>";var x=document.createElement("span");x.textContent="\u2715";x.style.cssText="cursor:pointer;color:#c00;padding:0 4px";x.onclick=function(){ED.versanti.splice(i,1);edRenderList();};r.appendChild(x);box.appendChild(r);});}
