...
function edSetTarget(p){ED.pass=p;ED.isNew=false;ED.summit={lat:p.lat,lon:p.lon};ED.versanti=(p.versanti||[]).map(function(v){return v;});document.getElementById("edNewFields").style.display="none";var rw=document.getElementById("edRenameWrap");if(rw){rw.style.display="block";document.getElementById("edRename").value=p.name||"";}document.getElementById("edTarget").innerHTML="Modifica: <b>"+esc(p.name)+"</b> &mdash; "+(p.versanti?p.versanti.length:0)+" versanti";edRenderList();map.setView([p.lat,p.lon],13);}
function edStartNew(){ED.isNew=true;ED.pass=null;ED.versanti=[];ED.summit=null;ED._await=true;document.getElementById("edNewFields").style.display="block";var rw=document.getElementById("edRenameWrap");if(rw)rw.style.display="none";document.getElementById("edTarget").innerHTML="Nuovo passo: clicca la CIMA sulla mappa (Disegna OFF).";edMsg("Clicca la cima del passo.");}
...
function edCommitStore(){
  ...
  if(ED.isNew){
    ...
    if(!ED.summit){edMsg("Manca la cima.");return false;}
    var nm=(document.getElementById("edName").value||"Nuovo passo").trim();
    id="manual-"+nm.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");
    entry={"new":true,name:nm,lat:ED.summit.lat,lon:ED.summit.lon,elevation:parseInt(document.getElementById("edElev").value)||Math.max.apply(null,ED.versanti.map(function(v){return v.endElevation;})),region:(document.getElementById("edRegion").value||"").trim(),status:"Aperto",versanti:ED.versanti};
  }
  else{
    if(!ED.pass){edMsg("Seleziona un passo.");return false;}
    id=ED.pass.id;entry={};
    var rn=document.getElementById("edRename"),newName=rn?rn.value.trim():"";
    if(newName&&newName!==ED.pass.name)entry.name=newName;        // rinomina passo (nome OSM sbagliato, traccia giusta)
    if(ED.versanti.length)entry.versanti=ED.versanti;
    if(!entry.versanti&&!entry.name){edMsg("Niente da salvare: modifica i versanti o cambia il nome.");return false;}
  }
  if(entry.versanti&&entry.versanti.length){
    entry.difficulty=Math.max.apply(null,entry.versanti.map(function(v){return estDiff(v.distance_km,v.endElevation-v.startElevation,v.endElevation);}));
    entry.cat=entry.versanti.map(function(v){return v.cat;}).filter(Boolean).sort(function(a,b){return catRank(b)-catRank(a);})[0]||null;
  }
  entry.updatedAt=today;entry.manual=true;
